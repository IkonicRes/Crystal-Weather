$(window).on("load", function() {
    ...
    function displayForecast(forecastData) {
      ...
      // Iterate over the forecastData array
      forecastData.forEach((day) => {
        // Create a new div for this day's forecast
        var dayElement = document.createElement('div');
        dayElement.className = "forecast-card";
    
        // Add the date, temp, humidity, and icon to the dayElement
        ...
        // Add the dayElement to the forecastElement
    ...
    function addCityToGallery(cityWeather) {
      ...
      galleryCell.on("click", function () {
        ...
        // Set the new city and fetch the weather again
    ...
    let condition;  // Declare global variable
  
    function weatherFetch() {
      var apiLocationUrl =
      "https://nominatim.openstreetmap.org/search?city=" +
      cityName +
      "&format=json&limit=1&appid=" +
        apiKey;
      ...
      fetch(apiLocationUrl)
        ...
        .then((data) => {
          ...
          if (data.length > 0) {
            ...
            fiveDayForecasts = fetch5DayForecast(latitude, longitude)
            setTimeout(function() {
              displayForecast(fiveDayForecasts);
            }, 1000);
            var apiWeatherUrl =
    ...
    $("#search-bar").keypress(async function (event) {
      ...
      if (keycode === 13) {
        ...
        cityName = toSearch;
        try {
          await weatherFetch(); // Assuming weatherFetch is an async function and sets a global 'condition' variable
        } catch (error) {
    ...
    $(".gallery").flickity({
      ...
      weatherFetch(blankWeather, sunnyClear); 
